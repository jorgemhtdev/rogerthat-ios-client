...
@interface GKImagePicker : NSObject {
    id<GKImagePickerDelegate> __weak delegate_;
    CGSize cropSize_;
    BOOL resizeableCropArea_;
}

@property (nonatomic, weak) id<GKImagePickerDelegate> delegate;
@property (nonatomic) CGSize cropSize; //default value is 320x320 (which is exactly the same as the normal imagepicker uses)
