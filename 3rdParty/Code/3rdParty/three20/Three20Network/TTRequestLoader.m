...
- (void)deliverDataResponse:(NSURL*)URL {
  ...
  if([[dataSplit objectAtIndex:0] rangeOfString:@"base64"].location == NSNotFound) {
    ...
    // Skip for now.
	_responseData = [NSMutableData dataWithData:[[dataSplit objectAtIndex:1] dataUsingEncoding:NSASCIIStringEncoding]];
  } else {
    _responseData = [NSData dataWithBase64EncodedString:[dataSplit objectAtIndex:1]];
  }
