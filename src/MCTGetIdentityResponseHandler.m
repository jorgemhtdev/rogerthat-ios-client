...
@implementation MCTGetIdentityResponseHandler

+ (MCTGetIdentityResponseHandler *)responseHandler
{
    return [[MCTGetIdentityResponseHandler alloc] init];
}

- (void)handleError:(NSString *)error
{
    T_BIZZ();
    LOG(@"Error response for GetIdentity request: %@", error);
}

- (void)handleResult:(MCT_com_mobicage_to_system_GetIdentityResponseTO *)result
{
    T_BIZZ();
    LOG(@"Result received for GetIdentity request");

    MCTSystemPlugin *plugin = (MCTSystemPlugin *) [MCTComponentFramework pluginForClass:[MCTSystemPlugin class]];

    [plugin.identityStore updateMyIdentity:result.identity withShortUrl:result.shortUrl];
}
